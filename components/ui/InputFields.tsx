  ...
  return (
    <Input
      ...
      classNames={{
        base: "md:min-w-[300px]",
        input:
          "focus:outline-none placeholder:text-xs placeholder:text-[#718096] text-sm",
        innerWrapper: "bg-transparent",
        inputWrapper:
...
export const RadioInput: React.FC<RadioInputProps> = ({ ...props }) => {
  return (
    <RadioGroup
      ...
      classNames={{ label: "text-sm text-[#344054] mb-1" }}
    >
      {props.options.map((option) => (
        <Radio
          key={option.value}
          value={option.value}
          classNames={{ label: "text-sm text-[#646A6A]" }}
        >
          {option.label}
        </Radio>
      ))}
...
//     >
//       {props.options?.map((item: any) => (
//         <SelectItem key={item.value || item.key}>{item.label}</SelectItem>
//       ))}
//     </Select>
//   );
// };
